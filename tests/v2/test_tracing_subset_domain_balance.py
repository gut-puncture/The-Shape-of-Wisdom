...
class TestTracingSubsetDomainBalance(unittest.TestCase):
    ...
    def test_subset_fails_when_trajectory_quota_is_infeasible(self) -> None:
        run_id = "test_v2_subset_trajectory_quota_infeasible"
        run_root = REPO_ROOT / "runs" / run_id
        out_root = run_root / "v2"
        if run_root.exists():
            shutil.rmtree(run_root)
        ...
        try:
            ...
            trajs = ["stable_correct", "stable_wrong", "unstable_correct"]
            types_rows = []
            for i in range(24):
            ...
            model_rep = report["models"]["Qwen/Qwen2.5-7B-Instruct"]
            self.assertFalse(bool(model_rep.get("pass")))
            self.assertIn("trajectory_quota_feasible", model_rep.get("failing_gates") or [])
        finally:
            if run_root.exists():
                shutil.rmtree(run_root)

    def test_subset_fails_when_difficult_domain_quota_is_infeasible(self) -> None:
        run_id = "test_v2_subset_difficult_domain_infeasible"
        run_root = REPO_ROOT / "runs" / run_id
        out_root = run_root / "v2"
        if run_root.exists():
            shutil.rmtree(run_root)
        ...
        try:
            ...
            with (run_root / "manifests" / "ccc_baseline.jsonl").open("w", encoding="utf-8") as f:
                for i in range(20):
                    domain = "hard_domain" if i < 2 else "easy_domain"
                    row = {
                        "prompt_uid": f"u{i}",
                        "example_id": f"e{i}",
                        "coarse_domain": domain,
            ...
            model_rep = report["models"]["Qwen/Qwen2.5-7B-Instruct"]
            self.assertFalse(bool(model_rep.get("pass")))
            self.assertIn("difficult_domain_feasible", model_rep.get("failing_gates") or [])
        finally:
            if run_root.exists():
                shutil.rmtree(run_root)
